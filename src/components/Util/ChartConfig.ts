...
export const getUnit = (currentKPI: KPIOption, valueType: string, forCSV: boolean = false) => {
	const unit = valueType === 'total' ? currentKPI.totalUnit : currentKPI.unit;
	return forCSV ? unit.replace(/CO₂/g, 'CO2').replace(/²/g, '2') : unit;
};

export const getUnitBracket = (currentKPI: KPIOption, valueType: string, forCSV: boolean = false) => {
	if (currentKPI.key === 'Urban Greening Factor') return getUnit(currentKPI, valueType, forCSV);
	return `(${getUnit(currentKPI, valueType, forCSV)})`;
};

// Common XAxis props for bar charts
export const getXAxisProps = (chart: string, currentKPI: KPIOption, valueType: string) => {
	let value = 'Year';
	...
	if (chart === 'Single Project' && currentKPI.key === 'Embodied Carbon') {
		axisLine = null;
	}
...
export const getYAxisProps = (chart: string, currentKPI: KPIOption, valueType: string) => {
