...
export const Chart = ({
  ...
  const [selectedKPI1, setSelectedKPI1] = useState("Operational Energy Total");
  const [selectedKPI2, setSelectedKPI2] = useState("Operational Energy Total");
  const [valueType, setValueType] = useState("average");
  const [showSingleProjectBenchmarks, setShowSingleProjectBenchmarks] =
    useState(false);
  const [selectedSubSector, setSelectedSubSector] = useState("");
  const [selectedBarChartBenchmark, setSelectedBarChartBenchmark] =
    useState("");
    ...
    switch (chartType) {
      ...
      case "Single Project":
        return (
          <SingleProject
            {...commonProps}
            showSingleProjectBenchmarks={showSingleProjectBenchmarks}
      ...
      case "Single Time":
        return (
          <SingleTime {...commonProps} selectedSubSector={selectedSubSector} />
        );
    }
  };
  ...
  const showSingleProjectSectorToggle =
    selectedKPI1 === "Upfront Carbon" &&
    valueType === "average" &&
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="flex justify-between items-center text-center mb-2">
          <h2 className="py-2">
            {getChartTitle(chartType, valueType, selectedKPI1, selectedKPI2)}
          </h2>
            ...
            {selectedKPI1 === "Total Embodied Carbon" &&
              ...
              chartType === "Single Project" && (
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    setShowSingleProjectBenchmarks(!showSingleProjectBenchmarks)
                  }
        ...
        {showSingleTimeSectorToggle && (
          <div className="mb-2 flex justify-between items-center py-4">
            <p
              className="text-sm font-medium mb-2 px-10"
              style={{ color: chartColors.pink }}
            >
              UKNZCBS Benchmarks
            </p>
            <div className="flex flex-wrap gap-2">
              {availableSubSectors.map((subSector) => (
                <Button
                  key={subSector}
                  variant={
                    selectedSubSector === subSector ? "default" : "outline"
                  }
                  size="sm"
        ...
        {showSingleProjectSectorToggle && (
          <div className="mb-2 flex justify-between items-center py-4">
            <p
              className="text-me font-medium mb-2 px-10"
              style={{ color: chartColors.pink }}
            >
              UKNZCBS Benchmarks
            </p>
            <div className="flex flex-wrap gap-2">
              <Button
                variant={
                  selectedBarChartBenchmark === "" ? "default" : "outline"
                }
                size="sm"
              ...
              {availableSubSectors.map((subSector) => (
                <Button
                  key={subSector}
                  variant={
                    selectedBarChartBenchmark === subSector
                      ? "default"
                      : "outline"
                  }
                  size="sm"
