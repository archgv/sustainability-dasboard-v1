...
export const ProjectPortfolio = ({ projects, isComparingToSelf = false, selectedRibaStages = [], primaryProject = '' }: ProjectPortfolioProps) => {
	...
	const getSectorDisplay = (typology: string) => {
		...
		return sectorMap[typology] || 'ERROR';
	};
	...
	const getProjectTypeColor = (type: string) => {
		return type === 'New Build' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800';
	};
						...
						<Card key={project.id} className="p-6 hover:shadow-lg transition-shadow duration-200">
							...
							<div className="space-y-3 mb-4">
								...
								<div className="flex items-center text-sm text-gray-600">
									...
									<span className="text-sm text-gray-600">{getRibaStageDisplay(project['Current RIBA Stage'])}</span>
								</div>
