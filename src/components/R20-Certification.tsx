...
export const Certification = ({ projects }: { projects: Project[] }) => {
  ...
  const getCertificationData = (certification: string) => {
    const ratings =
      certificationRatings[
        certification as keyof typeof certificationRatings
      ] || [];
    const data: {
      [key: string]: Project[];
    } = {};
  ...
  const maxCount = Math.max(
    ...Object.values(certificationData).map((projects) => projects.length),
    4
  );
  ...
  const getDisplayName = (project: Project) => {
    return `${project["id"]} ${project["Project Name"]}`;
  };
