	...
	const handleDownloadCSV = () => {
		...
		const csvContent =
			[
				...
				`KPI: ${currentKPI?.label} (${getDisplayUnit(true)})`,
				`Value Type: ${effectiveValueType}`,
				`Year Filter: ${yearFilter}`,
				'',
				`Sector,Projects,Average (${getDisplayUnit(true)}),Min (${getDisplayUnit(true)}),Max (${getDisplayUnit(true)}),Range (${getDisplayUnit(true)})`,
			].join('\n') + '\n';
