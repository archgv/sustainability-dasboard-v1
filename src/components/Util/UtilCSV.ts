...
export const exportChartToCSV = (options: ExportCSVOptions) => {
	...
	const transformedProjects = getProjectData(projects, valueType, selectedKPI1, selectedKPI2);

	// CSV headers
	const headers = ['Project Name', `${kpi1Config.key} (${getUnit(kpi1Config, valueType, true)})`];
	...
	if (chartType === 'Compare Two') {
		headers.push(`${kpi2Config?.key} (${getUnit(kpi2Config, valueType, true)})`);
	...
	transformedProjects.forEach((project) => {
		...
		if (chartType === 'Single Project' && selectedKPI1 === 'Embodied Carbon') {
			...
			const finalBiogenicValue = valueType === 'total' ? -Math.abs(biogenicValue * getGIA(project)) : -Math.abs(biogenicValue);
