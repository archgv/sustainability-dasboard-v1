...
export const SectorPerformance = ({ projects }: { projects: Project[] }) => {
  ...
  const availableYears = [...new Set(projects.map((p) => p["PC Year"]))].sort(
    (a, b) => b - a
  );

  // Filter projects by year if needed
  const filteredProjectsYear =
    yearFilter === "all"
      ? projects
      : projects.filter((project) => {
          const filterYear = parseInt(yearFilter);
          return project["PC Year"] == filterYear;
        });
  ...
  const sectorStats = filteredProjectsNull.reduce<SectorStatsMap>(
    (acc, project: Project) => {
      ...
      if (valueType === "total" && gia > 0) {
        ...
        if (
          selectedKPI === "Upfront Carbon" ||
          selectedKPI === "Embodied Carbon"
        ) {
          totalValue = totalValue / 1000; // Convert kg to tonnes
        } else if (
          selectedKPI === "Operational Energy" ||
          selectedKPI === "Operational Energy Gas"
        ) {
      ...
      {isExpanded && (
        <div className="">
          <div className="px-6">
            ...
            <SectorChart
              filteredProjects={filteredProjectsNull}
