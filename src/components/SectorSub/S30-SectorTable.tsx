  ...
  return (
    <Card className="shadow-inner">
      <div
        ...
        onClick={() => setIsExpandedSub(!isExpandedSub)}
      >
        <h2>Summary Statistics</h2>
        <ChevronDown
          className={`h-5 w-5 transition-transform duration-200 ${
            isExpandedSub ? "transform rotate-180" : ""
          }`}
      ...
      {isExpandedSub && (
        <Card className="rounded-[24px] p-2 mt-4">
          <div className="overflow-x-auto">
            <table
              className="w-full border-separate rounded-2xl overflow-hidden"
            ...
            >
                  ...
                  {valueType === "average" ? (
                    <>
                      <th className="px-4 py-2 text-center">
                        Average ({getUnit(currentKPI, valueType)})
                      </th>
                    ...
                    <>
                      <th className="px-4 py-2 text-center">
                        Total ({getUnit(currentKPI, valueType)})
                      </th>
                      <th className="px-4 py-2 text-center">Min</th>
                      <th className="px-4 py-2 text-center">Max</th>
              ...
              <tbody className="bg-pink-50">
                {SectorKeys.map((sector) => {
                  ...
                  const avg = stats
                    ? getAverage(stats.totalValue, stats.count)
                    : 0;
                  const min =
                    stats && stats.minValue !== Infinity
                      ? Math.round(stats.minValue)
                      : 0;
                  const max =
                    stats && stats.maxValue !== -Infinity
                  ...
                  return (
                    <tr
                      key={sector}
                      className="divide-y-2 divide-x-4 divide-white"
                    >
                      <td className="px-4 py-2 pl-8 text-left font-medium">
                        {sector}
                      </td>
