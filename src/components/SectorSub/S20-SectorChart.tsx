...
export const SectorChart = ({
  ...
  const chartData: ChartDataItem[] = SectorKeys.map((sector) => {
    ...
    if (selectedKPI === "Embodied Carbon" && stats) {
      const biogenicStats = filteredProjects.reduce<BiogenicStats>(
        (acc, project: Project) => {
          if (project["Primary Sector"] === sector) {
            const value = getProjectViewStage(project)["Biogenic Carbon"] || 0;
      ...
      biogenicValue =
        biogenicStats.count > 0
          ? Math.round(biogenicStats.totalValue / biogenicStats.count)
          : 0;
    }
  ...
  return (
    <Card className="shadow-inner mb-8">
      <h3
        className="font-medium mt-8 mb-2 text-center"
      ...
      <div className="h-[460px] flex justify-center" data-chart="sector-chart">
        <ResponsiveContainer {...getResponsiveContainerProps()}>
          <BarChart data={chartData} barGap={-50} {...getChartProps()}>
            ...
            <YAxis
              label={{
                value: `${currentKPI.key} ${getUnitBracket(
                  currentKPI,
                  valueType
                )}`,
                angle: -90,
            ...
            <Tooltip
              formatter={(value, name, props) => {
                if (selectedKPI === "Embodied Carbon") {
                  ...
                  return [
                    `${wholeLifeCarbon} ${getUnit(currentKPI, valueType)}`,
                    valueType === "total"
                      ? "Cumulative total Whole Life Carbon"
                      : "Average Whole Life Carbon",
                  ];
                }
                return [
                  `${formatNumber(Number(value))} ${getUnit(
                    currentKPI,
                    valueType
                  )}`,
                  valueType === "total" ? "Cumulative total" : "Average",
                ];
              }}
              ...
              content={({ active, payload, label }) => {
                if (
                  ...
                  const labelText =
                    valueType === "total" ? "Cumulative" : "Average";
                  return (
                    <div
                      style={{
                        ...getTooltipContainerStyle(),
                    ...
                    >
                      ...
                      >{`Sector: ${label}`}</p>
                      <p
                        style={{ margin: 0, color: chartColors.primary }}
                      >{`${labelText} Whole Life Carbon: ${formatNumber(
                        data.value
                      )} ${getUnit(currentKPI, valueType)}`}</p>
                      <p
                        style={{ margin: 0, color: chartColors.dark }}
                      >{`${labelText} Biogenic: ${formatNumber(
                        data.biogenicValue
                      )} ${getUnit(currentKPI, valueType)}`}</p>
                    </div>
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <p style={{ margin: 0, color: chartColors.primary }}>
                        {`${
                          valueType === "total" ? "Cumulative total" : "Average"
                        }: ${formatNumber(Number(payload[0].value))} ${getUnit(
                          currentKPI,
                          valueType
                        )}`}
                      </p>
            ...
            <Bar dataKey="value" barSize={50} radius={[6, 6, 0, 0]}>
              {chartData.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
                  fill={
                    sectorConfig[entry.sector as keyof typeof sectorConfig]
