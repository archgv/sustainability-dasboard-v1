...
export const exportSectorPNG = (options: ExportPNGOptions) => {
  ...
  logo.onload = () => {
    ...
    const title = `${prefix} ${currentKPI?.key} by sector (${getUnit(
      currentKPI,
      valueType
    )})`;
    ...
    img.onload = () => {
      ...
      const dateRangeText =
        yearFilter === "all"
          ...
          : `Date range: From ${yearFilter.replace("from-", "")}`;
      ...
      const tableHeaders =
        valueType === "per-sqm"
          ...
          : [
              "Sector",
              "No. of projects",
              `Cumulative total (${getUnit(currentKPI, valueType)})`,
              "Min",
              "Max",
