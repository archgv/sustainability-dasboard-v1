...
import { KPIMatrix, KPIOptionsFiltered, KPIKeysFiltered, KPIOptions } from './Key/KeyKPI';
		...
		<Card className="p-6">
			<div className="flex flex-wrap items-center justify-between gap-4">
				<div className="flex items-center gap-2">
					...
					{/* Value Type Selector */}
					{!['Biodiversity Net Gain', 'Urban Greening Factor'].includes(selectedKPI1) && (
				...
				<div className="flex items-center gap-2">
					...
					{chartType === 'Compare Two' && (
						<div>
							<Label htmlFor="kpi2" className="mb-4 block pl-6">
								KPI 2
							</Label>
							<Select value={selectedKPI2} onValueChange={setSelectedKPI2}>
								...
								<SelectContent>
									{KPIOptions.filter((kpi) => KPIMatrix[selectedKPI1]?.includes(kpi.key)).map((kpi) => (
										<SelectItem key={kpi.key} value={kpi.key}>
											{kpi.label}
										</SelectItem>
									))}
