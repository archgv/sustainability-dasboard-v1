...
export const SingleTime = ({ projects, selectedKPI1, valueType, isComparingToSelf = false, selectedSubSector }: SingleTimeProps) => {
	...
	const showBenchmarkOperationalEnergy = valueType === 'average' && selectedKPI1 === 'Operational Energy' && chartData.length > 0;
								...
								>
									...
									{projectData && (
										<>
											...
											<p className="text-sm" style={{ color: chartColors.dark }}>
												{kpi1Config.key}: {formatNumber(projectData.value as number)} {getUnit(kpi1Config, valueType)}
											</p>
										</>
									)}
										...
										<div className="mt-2 pt-2 border-t border-gray-200">
											{newBuildBenchmark && (
												<p className="text-sm" style={{ color: benchmarkColor }}>
													New Build: {formatNumber(newBuildBenchmark)} {getUnit(kpi1Config, valueType)}
												</p>
											)}
											{retrofitBenchmark && (
												<p className="text-sm" style={{ color: benchmarkColor }}>
													Retrofit: {formatNumber(retrofitBenchmark)} {getUnit(kpi1Config, valueType)}
												</p>
											)}
										</div>
									)}
